...
*  Copyright notice
*
*  (c) 2007 Sebastian Baumann <sb@sitesystems.de> for the initial idea and source
*  (c) 2008 Kurt Kunig <kurt.kunig@kupix.de> extended and modified version since sb_downloade V0.1.2
*  All rights reserved
*
...
// raus f�r TYPO3 V6.2.xx require_once(PATH_tslib.'class.tslib_pibase.php');


/**
 * Plugin 'Image Downloader' for the 'kk_downloader' extension.
 * with addons from Kurt Kunig kurt@kupix.de (Grafic of download/date & time)
 *
 * @author 1	Sebastian Baumann <sb@sitesystems.de>
 * @author 2	Kurt Kunig <kurt.kunig@kupix.de> for the revised Version based on source V0.1.2
 * @package	TYPO3
 * @subpackage	tx_kkdownloader
 */

class tx_kkdownloader_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	var $prefixId          = 'tx_kkdownloader_pi1';		// Same as class name
	var $scriptRelPath     = 'pi1/class.tx_kkdownloader_pi1.php';	// Path to this script relative to the extension dir.
   var $extPath           = 'typo3conf/ext/kk_downloader/';
	var $extKey            = 'kk_downloader';	// The extension key.
	var $pi_checkCHash     = true;
	var $filebasepath	     = "uploads/tx_kkdownloader/";
	var $defaultTemplate	  = "kk_download_template.html";
	// multilanguage
	var $langArr;
...
//							$showCRDate = 2;
      ...
      if ($this->debug > 1) print '<br><hr><br></div>';
	 ...
	 */
	function generateDownloadLinks($uid, $downloaddescription="1", $downloadIcon=""){
...
//					$filesize = filesize($downloadfile);
	 ...
	 * @param	array		$downloaddescription:1 = filename.fileextension, 2 = filename, 3 = fileextension
	 * @param	array		$downloadIcon: which downloadicon
	 * @return	modifies $markerarray by ref
	 */
...
//						$filesize = filesize($downloadfile);
	 ...
	 * Format completeCats
	 *
	 * @param	uid		$uid: ID of the calling object
	 * @return	return formated category-marker content
	 */
	function completeCats($uid, $cats){
      $cont = '';
      $uid  = $GLOBALS['TYPO3_DB']->quoteStr($uid,'tt_content');
      $cats = $GLOBALS['TYPO3_DB']->quoteStr($cats,'tt_content');
      if ($cats == '' || empty($cats)) $cats = 0;
      ...
      if ($this->debug > 1) print '<p style="color:#d0d;">$completeCats-><br>uid = '.$uid.'<br>cats = ' .$cats.'<br>sql =<br>' . $sql .'</p>';
...
#         $cont = $this->pi_getLL('category_text');
	...
	function downloadImage($image,$uid){
      ...
      if(!is_file($downloadfile)) exit;

//		if (!$downloadfile || $downloadfile = $this->filebasepath) exit;
		$valfilesize = filesize($downloadfile);
		$filename = $image;
...
// ab 13.07.2012 raus genommen wg. Schwierigkeiten bei einem User (php-Fehlermeldung)      ob_end_clean();
	   ...
	   $res = $GLOBALS['TYPO3_DB']->sql_query("UPDATE tx_kkdownloader_images
